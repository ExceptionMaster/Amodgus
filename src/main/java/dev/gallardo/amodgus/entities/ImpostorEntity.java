...
public class ImpostorEntity extends Monster implements GeoEntity {
	private AnimatableInstanceCache factory = GeckoLibUtil.createInstanceCache(this);

	private static final EntityDataAccessor<Integer> DATA_ID_TYPE_VARIANT = SynchedEntityData
			.defineId(ImpostorEntity.class, EntityDataSerializers.INT);
	...
	public static AttributeSupplier setAttributes() {
		return TamableAnimal.createMobAttributes().add(Attributes.MAX_HEALTH, 14.00).add(Attributes.ATTACK_DAMAGE, 7.0f)
				.add(Attributes.ATTACK_SPEED, 2.2f).add(Attributes.MOVEMENT_SPEED, 0.5f).build();
	}
